...
const Consultation = () => {
    ...
    useEffect(() => {
        async function fetchSessions() {
            try {
                const response = await api.get('sessions/?page=1&keyword=&filter=0&status=0');
                console.log('API Response:', response.data);
                
                // Cần convert object thành array
                if (response.data && response.data.sessions) {
                    const sessionsObject = response.data.sessions;
                    // Convert object thành array và thêm session_id
                    const sessionsArray = Object.keys(sessionsObject).map(sessionId => {
                        const session = sessionsObject[sessionId];
                        const startTime = session.time || '';
                        
                        // Tính endTime từ startTime + duration (phút)
                        let endTime = '';
                        if (startTime && session.duration) {
                            try {
                        ...
                        if (displayDate) {
                            try {
                                ...
                                displayDate = `${dayName}, ${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()}`;
                            } catch (e) {
                        ...
                        return {
                            ...
                            title: session.name || 'Chưa có tên',
                            name: session.name || 'Chưa có tên',
                            date: session.date || '',
                            displayDate: displayDate,
                            startTime: startTime,
                            endTime: endTime,
                            duration: `${session.duration || 0} phút`,
                            status: session.status === 3 ? 'upcoming' : 
                                    session.status === 2 ? 'finished' : 'upcoming',
                            tutor: session.tutor || '',
                    ...
                    console.log('Converted sessions array:', sessionsArray);
                    setSessions(sessionsArray);
                } else {
                    console.warn('No sessions data in response, using mock data');
        ...
        processed.sort((a, b) => {
            switch (sortOption) {
                ...
                case 'duration':
                    ...
                    return durationB - durationA; // Dài nhất lên đầu
                case 'date':
                default: 
        ...
        <div className="h-full font-sans">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 min-h-[600px]">
                ...
                <div className="space-y-4">
                    {displayedSessions && displayedSessions.length > 0 ? displayedSessions.map(item => (
