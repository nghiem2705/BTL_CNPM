...
import { 
  Calendar, Clock, Search, Plus, Download, 
  ChevronDown, Check, Loader // <--- Thêm icon Loader nếu thích
} from 'lucide-react';

// 1. SỬA IMPORT: Lấy sessionApi thay vì mockSessions
import { sessionApi } from '../../api'; 

const Home = () => {
  const navigate = useNavigate();

  // 2. SỬA STATE: Khởi tạo mảng rỗng và biến loading
  ...
  // 3. THÊM useEffect: Gọi API khi trang vừa mở
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        setLoading(true);
        // Gọi hàm getAll đã viết ở api/index.js (nó đã tự map dữ liệu rồi)
        const data = await sessionApi.getAll(); 
        setSessions(data);
      } catch (error) {
        console.error("Lỗi tải danh sách:", error);
        // Có thể thêm toast/alert báo lỗi ở đây
      } finally {
  ...
  // Xử lý Lọc & Sắp xếp (Logic này giữ nguyên, vì data đã được map chuẩn format)
  const getProcessedSessions = () => {
    let processed = [...sessions];
    ...
    processed.sort((a, b) => {
      switch (sortOption) {
        case 'title': return a.title.localeCompare(b.title);
        case 'duration': 
          // Lưu ý: duration bây giờ là chuỗi "90 phút", cần parse cẩn thận
          // Cách parse đơn giản: lấy số đầu tiên tìm thấy
          const getNum = (str) => parseInt(str) || 0;
          return getNum(b.duration) - getNum(a.duration);
        case 'date': 
  ...
  // 4. HIỂN THỊ LOADING: Trả về giao diện chờ nếu đang tải
  if (loading) {
    return (
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 min-h-[600px]">
           ...
           <div className="space-y-4">
             ...
             {displayedSessions.length === 0 && (
                <div className="text-center text-gray-400 py-10">Không tìm thấy buổi học nào.</div>
             )}
